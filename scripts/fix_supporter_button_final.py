#!/usr/bin/env python3
"""Final fix for supporter button - add pointer-events and z-index"""

import os
import re

# Old CSS pattern
old_pattern = r'\.btn-supporter\{display:inline-flex;align-items:center;gap:8px;background:var\(--accent\);color:var\(--accent-text\);border:0;border-radius:var\(--radius-sm\);padding:10px 16px;font-weight:700;font-size:14px;cursor:pointer;min-height:var\(--tap\);touch-action:manipulation;-webkit-tap-highlight-color:transparent;user-select:none;-webkit-user-select:none\}'

# New CSS with pointer-events and z-index
new_css = '.btn-supporter{display:inline-flex;align-items:center;gap:8px;background:var(--accent);color:var(--accent-text);border:0;border-radius:var(--radius-sm);padding:10px 16px;font-weight:700;font-size:14px;cursor:pointer;min-height:var(--tap);touch-action:manipulation;-webkit-tap-highlight-color:transparent;user-select:none;-webkit-user-select:none;pointer-events:auto;position:relative;z-index:10}'
...
def main():
    ...
    print("Adding pointer-events and z-index to supporter button...\n")
    ...
    print("Supporter button should now work on mobile!")
